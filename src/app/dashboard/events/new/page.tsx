...
import { createEvent, clearError } from "@/lib/slices/eventSlice";
...
export default function NewEventPage() {
  ...
  const onSubmit = async (data: EventFormData) => {
    ...
    const result = await dispatch(createEvent(formattedData));
    
    if (createEvent.fulfilled.match(result)) {
      toast.success("Event created successfully!", {
        description: `Your event "${data.name}" has been created.`,
      });
      router.push("/dashboard/events");
    } else if (createEvent.rejected.match(result)) {
      toast.error("Failed to create event", {
        description: result.payload as string || "An error occurred while creating the event.",
      });
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <p className="text-white/60">Fill in the details to create your new event</p>
      </motion.div>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {imagePreview && !uploadingImage && (
                <p className="text-xs text-white/50">Image uploaded: {imagePreview}</p>
              )}
              {errors.layoutImage && (
